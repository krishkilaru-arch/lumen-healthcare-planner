# Virtue Foundation Dataset (DAIS 2026)
...
* Explore postal geography ambiguity before joining to other datasets

> **Important** — row grain is post office, not PIN code. A single PIN can appear on multiple rows and may map to more than one district or state. A direct join on pincode will fan out rows unless you deduplicate or aggregate first. Always check cardinality before joining.
...
* Identify underserved districts where disease burden is high and facility coverage is low
