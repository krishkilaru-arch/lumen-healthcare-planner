...
const APP_CONFIG = {
  name: 'Lumen Virtue',
  plugins: ['analytics', 'lakebase', 'genie', 'serving'],
} as const;
...
const PLUGIN_PAGES: Record<string, PluginPage> = {
  analytics: {
    navLabel: 'Facilities',
    path: '/facilities',
    expectedTexts: ['Facility Health Explorer'],
  },
  lakebase: {
    navLabel: 'Worklist',
    path: '/worklist',
    expectedTexts: ['Outreach Worklist'],
  },
  genie: {
    navLabel: 'Genie',
    path: '/genie',
    expectedTexts: ['Ask Genie'],
  },
  serving: {
    navLabel: 'Assistant',
    path: '/assistant',
    expectedTexts: ['Lumen Virtue Assistant'],
  },
};
...
test('smoke test - app loads and displays home page', async ({ page }) => {
  ...
  await expect(page.getByText(APP_CONFIG.name, { exact: true })).toBeVisible();
  await expect(page.getByRole('heading', { name: /Healthcare facility intelligence/ })).toBeVisible();

  await expect(page.getByRole('link', { name: 'Home', exact: true })).toBeVisible();
  for (const [, plugin] of enabledPages) {
    await expect(page.getByRole('link', { name: plugin.navLabel, exact: true })).toBeVisible();
  }
});
